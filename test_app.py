...
class JobPortalTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_user_company = 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6IkpYN3U0NkUxMW9PSXRpVVpUemdKWiJ9..pVYHXx_txEYcH_LUYTWY3AToAzkeLhcBgs0ZQV1hx9BF8oN7_gxaveTMjFuBr4cq0A5nAnWlGIqep5a_hOdIcsxUnQwKHVkTQaBu6X10SxfJmMwuhHOLA0n9gaxPxDpLVFXNQFTRwVDW25p-TvHGib9VmNO_zZd9cbjM87go93P7djN8JByk79dGZ_HJ-SfZWp7u_UO7ItZAT9Ps6uIy1ekqAjOVIZqwonP7sv-53nldpHpe8V0UpqXd1f6ogqblAcR1VxedW4Utl2cRMaPc_LutibXJjEdHQOr6vafqMpyq2X5n3rkCg_LyJ1FszW6VbieE8DNSTlVZ_6SkixXzhQ'
        self.test_user_candidate = 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6IkpYN3U0NkUxMW9PSXRpVVpUemdKWiJ9..tw9zQSyNSwL15sj0ZMQzDFwKU7611vwChPwm_4eVbit8PJI8MPILcdSvbUt9s2Y_pkjOv94Y1QqOEvgVnuVviLcXf1SPl9wolzPQLKVzxpGGIp1xdE9WtclmLGlxWfWWqCiKEFNCiR5WzvsM_5UGkMMiupmVYOe2KNt3Hszgb4VbdSZED76KSBYrLdDFWA_hYUhzB-vzdN5baL_rpoYYGibZg883zKDqi2tuWwjAo439tuj4skPJTXmGVlqzaawA4B37LMtmS4zvbJNyAnwKoxFwu_SDNk-SUCpq7xkrB10KF6lWci0yFawOTPmmuy3K166czi5nWT2Wn_cOIB-8VQ'
    ...
    def test_add_new_company(self):
        ...
        res = self.client().post('/companies', json = self.new_company, \
            headers = {'Authorization': 'Bearer ' + self.test_user_company})
    ...
    def test_add_new_candidate(self):
        '''Tests successful request to post a new candidate profile into the database'''
        res = self.client().post('/candidates', json = self.new_candidate, \
            headers = {'Authorization': 'Bearer ' + self.test_user_candidate})
        data = json.loads(res.data)
    ...
    def test_add_new_application(self):
        '''Tests successful request to post a new candidate application by vacancy id'''
        res = self.client().post('/vacancies/1/applications', json = self.new_application, \
            headers = {'Authorization': 'Bearer ' + self.test_user_candidate})
        data = json.loads(res.data)
    ...
    def test_error_422_when_deleting_application(self):
        '''Tests error 422 when deleting application by invalid application id'''
    ...
    def test_error_401_no_authorization_header_company(self):
        '''Tests error 401 when authorization header is missing for company user'''
        res = self.client().post('/companies', json = self.new_company)
        data = json.loads(res.data)
    ...
    def test_error_401_no_authorization_header_candidate(self):
        '''Tests error 401 when authorization header is missing for candidate user'''
        res = self.client().post('/candidates', json = self.new_candidate)
        data = json.loads(res.data)
...
def suite():
    ...
    suite.addTest(JobPortalTestCase('test_error_404_not_found_when_add_company'))
    suite.addTest(JobPortalTestCase('test_add_new_candidate'))
    suite.addTest(JobPortalTestCase('test_error_404_not_found_add_new_candidate'))
    suite.addTest(JobPortalTestCase('test_add_new_vacancy'))
    suite.addTest(JobPortalTestCase('test_error_404_not_found_when_add_new_vacancy'))
    suite.addTest(JobPortalTestCase('test_add_new_application'))
    suite.addTest(JobPortalTestCase('test_error_406_not_acceptable_when_post_application'))
    suite.addTest(JobPortalTestCase('test_get_companies'))
    suite.addTest(JobPortalTestCase('test_error_404_not_found_when_get_companies'))
    suite.addTest(JobPortalTestCase('test_get_company_by_id'))
    suite.addTest(JobPortalTestCase('test_error_404_not_found_when_get_company_by_id'))
    suite.addTest(JobPortalTestCase('test_get_candidate_by_id'))
    suite.addTest(JobPortalTestCase('test_error_404_not_found_when_get_candidate_by_id'))
    suite.addTest(JobPortalTestCase('test_get_vacancies'))
    suite.addTest(JobPortalTestCase('test_error_404_not_found_when_get_vacancies'))
    suite.addTest(JobPortalTestCase('test_get_vacancy_by_id'))
    suite.addTest(JobPortalTestCase('test_error_404_not_found_when_get_vacancy_by_id'))
    suite.addTest(JobPortalTestCase('test_get_applications_by_candidate_id'))
    suite.addTest(JobPortalTestCase('test_error_404_not_found_when_get_applications_by_candidate_id'))
    suite.addTest(JobPortalTestCase('test_get_applications_by_vacancy_id'))
    suite.addTest(JobPortalTestCase('test_error_404_not_found_when_get_applications_by_vacancy_id'))
    suite.addTest(JobPortalTestCase('test_update_company_by_id'))
    suite.addTest(JobPortalTestCase('test_error_404_not_found_when_update_company'))
    suite.addTest(JobPortalTestCase('test_update_candidate_by_id'))
    suite.addTest(JobPortalTestCase('test_error_422_unprocessable_when_update_candidate'))
    suite.addTest(JobPortalTestCase('test_update_vacancy_by_id'))
    suite.addTest(JobPortalTestCase('test_error_422_unprocessable_when_update_vacancy'))
    suite.addTest(JobPortalTestCase('test_delete_application'))
    suite.addTest(JobPortalTestCase('test_error_422_when_deleting_application'))
    suite.addTest(JobPortalTestCase('test_delete_vacancy_by_id'))
    suite.addTest(JobPortalTestCase('test_error_422_unprocessable_when_delete_vacancy'))
    suite.addTest(JobPortalTestCase('test_delete_company_by_id'))
    suite.addTest(JobPortalTestCase('test_error_422_unprocessable_when_delete_company'))
    suite.addTest(JobPortalTestCase('test_delete_candidate_by_id'))
    suite.addTest(JobPortalTestCase('test_error_422_unprocessable_when_delete_candidate'))
    suite.addTest(JobPortalTestCase('test_error_401_token_not_found_company'))
    suite.addTest(JobPortalTestCase('test_error_401_no_authorization_header_company'))
    suite.addTest(JobPortalTestCase('test_error_401_token_not_found_candidate'))
    suite.addTest(JobPortalTestCase('test_error_401_no_authorization_header_candidate'))
